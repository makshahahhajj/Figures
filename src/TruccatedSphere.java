...
public class TruccatedSphere extends Figure{
    ...
    public TruccatedSphere(ArrayList<Point> Points, int n, int count1, int count2) {
        super(Points, n, count1, count2);
    }

    public String getArea() {
        double truncRadius = Math.sqrt(Math.pow(Points.get(CONSTS.FIRST).getX() - Points.get(CONSTS.THIRD).getX(), 2) + Math.pow(Points.get(CONSTS.FIRST).getY() - Points.get(CONSTS.FIRST).getY(), 2) + Math.pow(Points.get(CONSTS.THIRD).getZ() - Points.get(CONSTS.THIRD).getZ(), 2));
